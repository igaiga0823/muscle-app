...
const Login = () => {
  ...
  useEffect(() => {
    fetch("https://iganami1106.com/muscle_api/index.cgi/loginstart", {
      method: "GET",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const dataSend = (initialPassword) => {
    ...
    console.log(after_password);
    fetch(
      `https://iganami1106.com/muscle_api/index.cgi/signin?user_name=${user_name}&after_password=${complete_password}&session_id=${sessionID}`,
      { method: "GET" }
    )
      .then((res) => res.json())
      .then((data) => {
